...
export type TooltipItem={itemId?:number;name?:string;itemLevel?:number;slot?:string;source?:string;enchant?:string;gems?:string[];bonusIds?:number[];className?:string;spec?:string};
...
const endpoint=(item:TooltipItem)=>{
  ...
  if(item.className)params.set('className',item.className);
  if(item.spec)params.set('spec',item.spec);
...
export function GameItemIcon({item,children,className='item-icon'}:Props){
  ...
  const key=`${item.itemId||'none'}|${item.itemLevel||''}|${item.bonusIds?.join('/')||''}|${item.className||''}|${item.spec||''}`;
...
export function GlobalItemTooltip(){
  ...
  const itemFor=(icon:HTMLElement):TooltipItem & {effect?:string}=>{
    ...
    return {itemId:itemId||undefined,name,slot:icon.getAttribute('aria-label')||undefined,enchant:icon.dataset.enchant,gems:icon.dataset.gems?.split('/').filter(Boolean),className:document.documentElement.dataset.tooltipClass,spec:document.documentElement.dataset.tooltipSpec,effect};
  };
  const show=(icon:HTMLElement)=>{const rect=icon.getBoundingClientRect(),width=Math.min(350,window.innerWidth-20);setPosition({left:Math.max(10,Math.min(window.innerWidth-width-10,rect.right+12)),top:Math.max(10,Math.min(window.innerHeight-20,rect.top))});setTarget(icon);const item=itemFor(icon),url=endpoint(item);if(!url){if(item.effect){setData({status:'exact',lines:[{left:item.name,kind:'name'},{left:item.effect,kind:'enchant'}]});}else{setData({status:'missing',lines:[{left:item.name||'Item information unavailable',kind:'name'},{left:'This icon has no captured item link.',kind:'missing'}]});}return;}const key=`${item.itemId}|${item.itemLevel||''}|${item.className||''}|${item.spec||''}`;const prior=cache.get(key);if(prior){setData(prior);return;}setData(undefined);fetch(url,{cache:'no-store'}).then(r=>r.ok?r.json():undefined).then(value=>{if(value){cache.set(key,value);setData(value);}}).catch(()=>undefined);};
  useEffect(()=>{

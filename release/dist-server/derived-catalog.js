...
const sourceSeed = () => JSON.parse(readFileSync(sourceSeedPath, 'utf8'));
function setup(db) {
...
export function installDerivedCatalog(packagePath = defaultPackage) {
    if (!existsSync(packagePath))
        return { installed: false, reason: 'No DB2 derived package found.' };
    const value = JSON.parse(readFileSync(packagePath, 'utf8'));
    if (value.schema !== 3 || value.status !== 'derived' || !Array.isArray(value.items) || !Array.isArray(value.enhancements))
        throw new Error('Unsupported DB2 derived package. Run catalog:db2:prefill again with the current tool.');
    const catalog = pathsForCatalog();
    if (!existsSync(catalog.catalogPath))
        return { installed: false, reason: 'Refresh the Blizzard catalog before installing DB2 metadata.' };
    ...
    try {
        ...
        db.transaction(() => {
            ...
            for (const row of value.journalDrops)
                dropPresence.run(row.itemId, value.clientBuild, value.generatedAt, row.journalEncounterId);
            for (const row of value.enhancements)
                enhancement.run(row.enchantId, row.name, row.iconFileDataId || null, row.type, JSON.stringify(row.slots || []), value.clientBuild, value.generatedAt, row.status);
            for (const row of seed.categories)
                category.run(row.id, seed.season, row.name, row.captureMode, JSON.stringify(row.tracks));
        ...
        db.close();
        const manifest = existsSync(catalog.manifestPath) ? JSON.parse(readFileSync(catalog.manifestPath, 'utf8')) : {};
...
export function derivedCatalogHealth() {
    ...
    try {
        const diagnostic = db.prepare(`SELECT payload FROM catalog_build_diagnostics WHERE id='db2-derived'`).get();
        const categories = db.prepare('SELECT id,name,capture_mode as captureMode,tracks FROM source_categories ORDER BY name').all().map((x) => ({ ...x, tracks: JSON.parse(x.tracks) }));
        const enhancementRows = db.prepare(`SELECT count(*) as total FROM derived_enhancements`).get();
        const dropRows = db.prepare(`SELECT count(*) as total FROM derived_drop_presence`).get();

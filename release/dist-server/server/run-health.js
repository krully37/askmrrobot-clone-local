...
export function assessRunHealth(input) {
    ...
    let state = 'healthy', message;
    if (active && input.status === 'running' && !input.activity && progressAgeMs >= 30_000) {
        state = 'orphaned';
        message = 'The dashboard is no longer connected to the local SimC process for this run. It cannot resume tracking this job.';
    }
    else if (active && outputAgeMs !== undefined && outputAgeMs >= STALLED_AFTER_MS) {
        state = 'stalled';
        message = 'SimC has produced no output for five minutes. This run may be stuck; cancel it and run it again.';
    }
    else if (active && ((outputAgeMs !== undefined && outputAgeMs >= WATCH_AFTER_MS) || (!input.activity && progressAgeMs >= WATCH_AFTER_MS))) {
        state = 'watch';
        message = 'No new simulation activity has been observed for two minutes. The estimate may be unreliable while this batch is quiet.';
    }

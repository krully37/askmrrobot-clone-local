...
export function canDualWield(className, spec) { return dualWieldSpecs.has(`${className.toLowerCase()}:${spec.toLowerCase()}`); }
export function parseProfile(raw) {
    ...
    if (!actor)
        throw new Error('This does not look like a SimulationCraft addon export. Include the character class line (for example, warrior=Name).');
...
const sourceHeader = (line) => /great\s+vault|weekly\s+reward(?:s)?(?:\s+choices)?/i.test(line) ? 'vault' : /custom\s+candidates/i.test(line) ? 'custom' : /gear\s+from\s+(bags|bank)|bags/i.test(line) ? 'bags' : undefined;
function candidateFromLine(line, source, index, comment) {
    const body = line.replace(/^\s*#\s*/, '').trim();
    const m = body.match(/^([a-z_0-9]+)=(.+)$/i);
    if (!m || !slots.has(m[1].toLowerCase()))
        return;
...
export function parseInventory(clean) {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const header = sourceHeader(line);
        if (header) {
            source = header;
            pendingItem = undefined;
            continue;
        }
        const item = candidateFromLine(line, source, sequence++, pendingItem);
        if (item) {
            ...
            if (item.itemId === 248583 && typeof item.rawLine === 'string') {
                ...
                if (loaMatch) {
                    ...
                    const loas = { '13183': 'Critical Strike', '13184': 'Haste', '13185': 'Mastery', '13186': 'Versatility' };
        ...
        if (comment && !/^\s*#\s*(?:Saved Loadout:|talents=)/i.test(line)) {
            const itemLevel = Number(comment[1].match(/\s+\((\d+)\)\s*$/)?.[1]) || undefined;
            pendingItem = { name: comment[1].replace(/\s+\(\d+\)\s*$/, '').trim(), itemLevel };
        }
        const loadout = line.match(/^\s*#\s*Saved Loadout:\s*(.+)$/i);
        if (loadout) {
            ...
            if (talentsValue)
                talents.push({ id: `saved-${talents.length}`, name: loadout[1].trim(), talents: talentsValue, spec: specValue, hero_talents: heroValue, selected: false });
        }
    }
    ...
    if (active)
        talents.unshift({ id: 'active', name: 'Active loadout', talents: active, spec: activeSpec, hero_talents: activeHero, selected: true });
...
export function scenarioOverlay(s) {
    ...
    if (s.bloodlust === 'time')
        lines.push(`bloodlust_time=${s.bloodlustValue ?? 0}`);
    if (s.bloodlust === 'health')
        lines.push(`bloodlust_percent=${s.bloodlustValue ?? 25}`);
    if (s.bloodlust === 'end')
        lines.push('bloodlust_time=0');
    if (!s.raidBuffs)
        lines.push('optimal_raid=0');
    const selections = s.consumableSelections || {};
    for (const key of ['food', 'flask', 'potion'])
        if (selections[key])
            lines.push(`${key}=${selections[key]}`);
    ...
    if (temporary.length)
        lines.push(`temporary_enchant=${temporary.join('/')}`);
...
export function buildInput(profile, scenario, threads) {

...
function generateActionsForSlot(candidate, crests, sparks, discount) {
    const actions = [];
...
export function generateAdvisorPermutations(inventory, req) {
    const equipped = inventory.candidates.filter((c) => c.source === 'equipped');
    const slotActions = equipped.map((c) => ({
        candidate: c,
        actions: generateActionsForSlot(c, req.crests, req.sparks, req.discounts[c.slot] || false)
    }));
    const validPermutations = [];
    let iterations = 0;
    // Recursive generation
    function recurse(index, currentActions, currentCosts, currentSparks, currentIlvlGain) {
        iterations++;
        if (iterations > 100000)
            throw new Error("Too many upgrade permutations. Reduce crest count or apply discounts to narrow the search space.");
        if (validPermutations.length > 20000)
            return; // limit to prevent memory/cpu exhaustion
        ...
        for (const action of actions) {
            ...
            if (ok && action.costType !== 'none') {
                ok = newCosts[action.costType] <= (req.crests[action.costType] || 0);
            }
            if (ok) {
...
export function buildAdvisorLoadouts(inventory, req) {
    ...
    if (!talent)
        throw new Error('No talent build found in profile');

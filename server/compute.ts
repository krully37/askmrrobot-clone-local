...
async function windowsCpuUsage():Promise<number|undefined>{
  return await new Promise(resolve=>execFile('powershell.exe',['-NoProfile','-NonInteractive','-Command','(Get-CimInstance Win32_Processor | Measure-Object -Property LoadPercentage -Average).Average'],{windowsHide:true,timeout:4000},(error,stdout)=>{
    const value=Number(String(stdout).trim()); resolve(!error&&Number.isFinite(value)?clamp(value,0,100):undefined);
  }));
}

export async function computeCapacity():Promise<ComputeCapacity>{
...
export async function safeThreads(requested:unknown):Promise<{threads:number;capacity:ComputeCapacity;clamped:boolean}>{
  const capacity=await computeCapacity(); const requestedNumber=Number(requested);

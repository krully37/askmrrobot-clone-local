...
function validSelection(chosen:GearCandidate[]){const ids=new Set<string>(),unique=new Set<string>();let twoHand=false,empty=false;for(const candidate of chosen){if(candidate.id===emptyOffHand.id){empty=true;continue;}const baseId=candidate.id.split('~')[0];if(ids.has(baseId))return false;ids.add(baseId);if(candidate.uniqueKey){if(unique.has(candidate.uniqueKey))return false;unique.add(candidate.uniqueKey);}if(candidate.slot==='main_hand'&&candidate.handedness==='two-hand')twoHand=true;}return twoHand===empty;}

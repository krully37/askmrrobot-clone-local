...
describe('catalog pipeline configuration', () => {
  ...
  it('does not report configured without both credentials', () => {
    ...
    delete process.env.BLIZZARD_CLIENT_ID; delete process.env.BLIZZARD_CLIENT_SECRET;
    expect(configured()).toBe(false);
    if (originalId) process.env.BLIZZARD_CLIENT_ID = originalId;
    if (originalSecret) process.env.BLIZZARD_CLIENT_SECRET = originalSecret;
  });
});

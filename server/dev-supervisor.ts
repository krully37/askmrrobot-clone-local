...
for(const target of ['server','tsconfig.json']) { try { watch(resolve(root,target),{recursive:target==='server'},restartForSourceChange); } catch { /* Vite remains available if a platform cannot watch recursively. */ } }

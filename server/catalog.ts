...
/**
 * The derived_* tables only exist once a DB2 package has been installed, and it
 * is optional. They carry icon ids and item metadata, so a missing one degrades
 * presentation rather than correctness; querying it blind throws SQLITE_ERROR.
 */
function hasTable(db:Database.Database,name:string){return Boolean(db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(name));}

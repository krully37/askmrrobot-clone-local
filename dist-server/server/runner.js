 ...
 */
export function profileResults(json) {
    ...
    return values
        ...
        .filter((x) => x.name !== 'profileset' && Number.isFinite(x.dps));
}
export async function execute(id, input, options = {}) {
    const rt = runtime();
    if (!rt.path)
        throw new Error('SimulationCraft was not found. Set SIMC_PATH to simc.exe or put it in runtime/simc.exe.');
    ensureReports();
    const prefix = join(paths.reports, `run-${id}${options.suffix ? `-${options.suffix}` : ''}`);
    const inputPath = `${prefix}.simc`;
    const reportPath = `${prefix}.html`;
    ...
    return await new Promise((resolve, reject) => { const p = spawn(rt.path, [inputPath, `html=${reportPath}`, `json2=${jsonPath}`], { windowsHide: true }); const now = new Date().toISOString(); activity.set(id, { startedAt: now, lastOutputAt: now, stdoutBytes: 0, stderrBytes: 0, pid: p.pid }); running.set(id, p); let out = ''; let err = ''; let lineBuffer = ''; let currentProfile = ''; let completedInBatch = 0; p.stdout.on('data', d => { const chunk = d.toString(); out += chunk; const state = activity.get(id); if (state) {
        ...
        let latestCompleted = -1, latestTotal = -1;
        for (const part of parts) {
            const m = part.match(/Generating (Profileset: .*?|Baseline:) (\d+)\/(\d+)/);
            if (m) {
                const pName = m[1];
        ...
        if (options.finalize !== false) {
            const result = parseSimcResult(jsonPath);
            if (result)
                saveRunResult(id, result);
            updateRun(id, { status: 'completed', reportPath, summary: profilesets.length ? `${profilesets.length} Top Gear profiles completed` : dps ? `DPS ${dps}` : 'Completed', completedAt: new Date().toISOString() });
        }
        resolve({ reportPath, jsonPath, profilesets, elapsedMs: Date.now() - started });
    }
    else {
        if (options.finalize !== false)
            updateRun(id, { status: 'failed', summary: (err || out || `SimC exited ${code}`).slice(-2000), completedAt: new Date().toISOString() });
        reject(new Error(err || out || `SimC exited ${code}`));
    } }); });
}
export function cancel(id) { const p = running.get(id); if (!p)
    return false; p.kill(); updateRun(id, { status: 'cancelled', completedAt: new Date().toISOString() }); return true; }

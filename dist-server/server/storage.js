 ...
 */
export function hasLocalState(root) {
    const files = [
...
export function recoverLocalState(target, candidates) {
...
export function storageRoot(environment = process.env, cwd = process.cwd(), home = homedir()) {
    ...
    if (!hasLocalState(legacyWebRoot) && hasLocalState(fallbackRoot))
        return fallbackRoot;

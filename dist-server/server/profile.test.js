...
describe('SimulationCraft profile support', () => {
    ...
    it('generates guided scenario options and preserves raw override', () => {
        const scenario = { name: 'Cleave', fightStyle: 'Patchwerk', duration: 180, variation: 20, targets: 3, bloodlust: 'time', bloodlustValue: 20, raidBuffs: true, consumables: true, rawOverride: 'iterations=5000' };
    ...
    it('expands selected gem and enchant alternatives into exact SimC item variants', () => {
        const candidate = { id: 'head', slot: 'head', name: 'Helm', rawLine: 'head=helm,id=1,gem_id=10', source: 'equipped', selected: true, locked: true };
        const variants = withEnhancementVariants([candidate], [{ id: 'gem-20', type: 'gem', name: 'Gem', slots: ['head'], simcFragment: 'gem_id=20' }, { id: 'ench-30', type: 'enchant', name: 'Enchant', slots: ['head'], simcFragment: 'enchant_id=30' }], ['gem-20', 'ench-30'], true);
    ...
    it('preserves existing enhancements unless replacement is explicitly enabled', () => {
        const candidate = { id: 'head', slot: 'head', name: 'Helm', rawLine: 'head=helm,id=1,gem_id=10/11,enchant_id=30', source: 'equipped', selected: true, locked: false };
        const enhancements = [
            { id: 'gem-20', type: 'gem', name: 'Gem', slots: ['head'], simcFragment: 'gem_id=20' },
            { id: 'gem-21', type: 'gem', name: 'Gem', slots: ['head'], simcFragment: 'gem_id=21' },
            { id: 'ench-40', type: 'enchant', name: 'Enchant', slots: ['head'], simcFragment: 'enchant_id=40' },
        ];
...
# talents=cleaveHash`;
    ...
    it('keeps Vault rewards mutually exclusive in planned loadouts', () => {
        const inventory = parseInventory(raw);
        const request = { candidateIds: inventory.candidates.map(c => c.id), lockedSlots: [], talentIds: [inventory.talents[0].id], threads: 1, limit: 100, profileId: 1, scenario: { name: 'x', fightStyle: 'Patchwerk', duration: 60, variation: 0, targets: 1, bloodlust: 'pull', raidBuffs: true, consumables: true, rawOverride: '' }, confirmLarge: true };
    ...
    it('allows a dual-wield spec to assign distinct one-handed weapons to either hand', () => {
        ...
        inventory.candidates.forEach(candidate => { if (['main_hand', 'off_hand'].includes(candidate.slot)) {
            candidate.handedness = 'one-hand';
            candidate.locked = false;
        } });
        const request = { candidateIds: inventory.candidates.map(c => c.id), lockedSlots: [], talentIds: ['active'], enhancementIds: [], threads: 1, limit: 100, profileId: 1, scenario: { name: 'x', fightStyle: 'Patchwerk', duration: 60, variation: 0, targets: 1, bloodlust: 'pull', raidBuffs: true, consumables: true, rawOverride: '' }, confirmLarge: true };
    ...
    it('writes exactly one profileset item line for each final gear slot', () => {
        const plans = [{ name: 'hands', source: 'bags', talent: { id: 'active', name: 'Active', talents: 'x', active: true, selected: true }, candidates: [
                    { id: 'mh', slot: 'main_hand', name: 'Main', rawLine: 'main_hand=main,id=1', source: 'equipped', selected: true, locked: false },
                    { id: 'oh', slot: 'off_hand', name: 'Off', rawLine: 'main_hand=off,id=2', source: 'equipped', selected: true, locked: false },
                ] }];

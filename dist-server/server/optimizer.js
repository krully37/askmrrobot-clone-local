...
function validSelection(chosen) { const ids = new Set(), unique = new Set(); let twoHand = false, empty = false; for (const candidate of chosen) {
...
function enumerate(candidates, locked, dualWield, limit) { const slots = canonicalSlots.filter(slot => slotPool(slot, candidates, locked, dualWield).length > 0), output = []; const walk = (i, selected) => { if (output.length >= limit)
...
function counted(candidates, locked, dualWield) { return enumerate(candidates, locked, dualWield, 10_000_001).length; }
...
export function planOptimization(inventory, request, enhancements = []) { const preview = previewOptimization(inventory, request, enhancements); if (!request.confirmLarge && preview.combinations > request.limit)
    ...
    throw new Error('Select at least one talent build.'); const locked = new Set(request.lockedSlots), bags = selected.filter(c => c.source !== 'vault'), vault = selected.filter(c => c.source === 'vault'), perTalent = Math.max(1, Math.floor(request.limit / talents.length)), plans = []; for (const talent of talents) {
    enumerate(bags, locked, inventory.dualWieldCapable, perTalent).forEach((candidates, i) => plans.push({ name: `bags_${talent.id}_${i + 1}`, source: 'bags', talent, candidates }));
    for (const candidate of vault)
        enumerate([...bags, candidate], locked, inventory.dualWieldCapable, perTalent).filter(candidates => candidates.some(x => x.id === candidate.id)).forEach((candidates, i) => plans.push({ name: `vault_${candidate.id}_${talent.id}_${i + 1}`, source: 'vault', vaultCandidateId: candidate.id, talent, candidates }));

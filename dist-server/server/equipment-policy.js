...
const normal = (value) => value.toLowerCase().replace(/_/g, '').trim();
const inventorySlots = { 1: 'head', 2: 'neck', 3: 'shoulder', 5: 'chest', 7: 'legs', 8: 'feet', 9: 'wrist', 10: 'hands', 11: 'finger1', 12: 'trinket1', 13: 'main_hand', 14: 'off_hand', 16: 'back', 17: 'main_hand', 21: 'main_hand', 22: 'off_hand', 23: 'off_hand' };
/** DB2 supplies numeric inventory types even when the catalog has no text slot. */
export function resolveSimulationSlot(slot, inventoryType) { return slot !== 'unknown' ? slot : inventoryType == null ? 'unknown' : inventorySlots[inventoryType] || 'unknown'; }
export function evaluateEquipment(className, spec, slot, metadata) {
    ...
    if (metadata.classId != null && metadata.classId !== 2 && metadata.classId !== 4)
        return { eligible: false, confidence: 'confirmed', reason: `DB2 item class ${metadata.classId} is not equippable gear.`, ...base };
    if (universalSlots.has(slot))
        return { eligible: true, confidence: 'confirmed', ...base };
    const rule = policy[normal(className)];
    if (!rule)
        return { eligible: true, confidence: 'unknown', reason: `No equipment policy for ${className}.`, ...base };
    if (metadata.classId == null || metadata.subclassId == null)
        return { eligible: true, confidence: 'unknown', reason: 'DB2 class/subclass metadata is unavailable.', ...base };
    if (metadata.classId === 4)
        return metadata.subclassId === rule.armor
    ...
    if (metadata.classId === 2)
        return rule.weapons.includes(metadata.subclassId)
            ? { eligible: true, confidence: 'confirmed', ...base }

export interface OmniumFolioRune { talentId: number; spellId: number; name: string; }
export const OMNIUM_FOLIO_ROWS: readonly (readonly OmniumFolioRune[])[] = [
...
export function omniumFolioVariants(selected: number[]) {
  const choices = OMNIUM_FOLIO_ROWS.map((row) => row.filter((rune) => selected.includes(rune.talentId)));
  if (choices.some((row) => row.length === 0)) return [] as number[][];
  return choices.reduce<number[][]>((variants, row) => variants.flatMap((variant) => row.map((rune) => [...variant, rune.talentId])), [[]]);
}
